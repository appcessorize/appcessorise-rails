...
### Security — dependency bumps (branch `security-gem-bumps`, NOT yet deployed)
...
**Status:** all checks green — full suite **26 runs, 0 failures**, Brakeman clean, bundler-audit clean, RuboCop clean. **Left on the branch for review; not merged/deployed** — a Devise + Puma major-version bump to a live commerce site warrants explicit sign-off. Merge `security-gem-bumps` → `main` to ship (same auto-deploy path; no migrations).
